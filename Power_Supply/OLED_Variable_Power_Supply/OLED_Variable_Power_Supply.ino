...
uint16_t voltage_adjust = 10000;
uint8_t current_adjust = 100;




uint32_t voltage_set =9000;
...
uint32_t mod_adj_voltage = 100000;
uint32_t div_adj_voltage = 10000;
  ...
  up_state_curr = digitalRead(UP);
   if((up_state_prev == 1) && (up_state_curr == 0))
   {
     if(mode){
        ...
        if(voltage_set < 20000) voltage_set += voltage_adjust;
     }
   }
   ...
   if((right_state_prev == 1) && (right_state_curr == 0))
   {
     if(mode){
      ...
      digit_voltage_adjust_state = (digit_voltage_adjust_state + 1)%4;
      voltage_adjust = (voltage_adjust == 10) ? 10000 : (voltage_adjust/10);
     }
   }
   right_state_prev = right_state_curr;

   if(voltage_set >= 4000) analogWrite(18,(1023 - (voltage_set*boost_increment/1000)-63)); // -53?
   else analogWrite(18,1023);
   
   //linear_divider = voltage_set * linear_increment* 11 /(1100 + 5100);
   if(voltage_set <= 4000) analogWrite(14, voltage_set * linear_increment* 11 /(1100 + 5100)); 
   else if (voltage_set >= 18500) analogWrite(14, (voltage_set * linear_increment* 11 /(1100 + 5100)+((voltage_set-4000)/1000)*((voltage_set-4000)/1000))); 
   else analogWrite(14, (voltage_set * linear_increment* 11 /(1100 + 5100)+((voltage_set-4000)/1000))); //((voltage_set-4000)/1000))
...
void drawActualVoltage(){
   voltage_read = analogRead(VOLTAGE_READ_PIN)* 3300 * (182+36) / 36 / 1024;
   //voltage_read = voltage_read  3300 * (182+36) / 36 / 1024;
   ...
   u8x8.draw2x2String(4, 0, ".");
     for (uint8_t i = 0; i < 4; i++){
        ...
        if(i== 0 || i ==1) u8x8.draw2x2String(2*i, 0, buf);
...
void drawActualCurrent(){
  // current_read = analogRead(CURRENT_READ_PIN); //-5
   current_read = (analogRead(CURRENT_READ_PIN)-5)*3300*10/50/1024;
   mod_adj_current = 1000;
   div_adj_current = 100; 
